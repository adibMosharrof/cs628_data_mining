...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, outputs, target, size_average=True):
        distances = (outputs[0] - outputs[1]).pow(2).sum(1)  # squared distances
        losses = 0.5 * (
...
class TripletLoss(nn.Module):
    ...
    def forward(self, outputs, size_average=True):
